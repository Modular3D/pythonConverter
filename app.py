...
@converter_bp.route('/glb-usdz', methods=['POST'])
def convert_glb_to_usdz():
    # Check if the request contains a file named 'glb'
    if 'glb' not in request.files:
        return jsonify({'error': 'GLB file is required'}), 400
    ...
    temp_glb_file_path = f'{original_base_name}_{timestamp}.glb'
    
    # Save the uploaded GLB file to a temporary location
    glb_file.save(temp_glb_file_path)
    
    # Load the GLB file and convert it to USDZ
    scene = a3d.Scene.from_file(temp_glb_file_path)
    output_usdz_file_path = f'{original_base_name}_{timestamp}.usdz'
    scene.save(output_usdz_file_path)
    ...
    # Upload both the uploaded GLB file and the output USDZ file to S3
    with open(temp_glb_file_path, 'rb') as original_file, open(output_usdz_file_path, 'rb') as converted_file:
        files = [
            ('file', (temp_glb_file_path, original_file, 'model/gltf-binary')),
            ('file', (output_usdz_file_path, converted_file, 'model/vnd.usdz+zip'))
        ]
        response = requests.post('https://api.modularcx.link/global-functions-api/s3/upload-to-s3', data=form_data, files=files)
